...
function transition(action,payload={}){
 ...
 else if(action==='travel'){if(!state.solved.marriage||state.trips.length!==4||!state.chats.includes('tripChat'))fail('原始差旅单与项目排期尚未完成一致性核对。');else if(DATA.trips.some(t=>payload[t.city]!==t.answer))fail('人员与项目编号不一致。');else{state.solved.travel=true;addUnique(state.reviewed,14);note('travel','三笔原始差旅由实际出差人报销；广州为张佳诚的在岗记录。四城信息被重复关联。');}}
