...
function transition(action,payload={}){
 ...
 else if(action==='chat'){const c=DATA.chats.find(c=>c.id===payload.id);if(!c||!allowed(c.gate))fail('历史记录尚未开放。');else{addUnique(state.chats,c.id);note('chat:'+c.id,'已查看：'+c.channel+' / 本月'+c.date+'日。');if(c.id==='migrationChat'&&state.migrationSource){state.solved.migration=true;addUnique(state.reviewed,18);note('migration','OP-026临时材料单被迁移程序解释为员工主档案，七份材料副本因此关联到张佳诚。原业务记录与实际人员保持不变。');}if(c.id==='borrowChat')addUnique(state.reviewed,23);if(c.id==='recentChat')addUnique(state.reviewed,27);}}
 ...
 else if(action==='migrationSource'){if(!state.migrationLog||!state.attachments.includes('box')||!state.attachments.includes('catReview'))fail('导入附件与操作日志尚未形成完整批次。');else if(payload.person!=='刘一颖')fail('所选人员与终端登录账号不一致。');else{state.migrationSource=true;note('migrationSource','18日批次由刘一颖执行，来源为OP-026临时材料暂存模板。');}}
 ...
 else if(action==='compensation'){if(!['manual','automatic'].every(id=>state.compDocs.includes(id)))fail('两类薪资项目尚未完成核对。');else if(+payload.manual!==6000||+payload.automatic!==1150)fail('人工补发与自动福利的金额分类不一致。');else{state.solved.compensation=true;note('compensation','历史数据异常专项补发6000元；系统自动福利1150元另计。补偿已进入薪资草稿，个人资料仍未恢复。');}}
